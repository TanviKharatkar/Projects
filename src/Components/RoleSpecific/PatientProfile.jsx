...
const PatientProfile = () => {
  const [formData, setFormData] = useState({
    user_id: "", // Assuming user_id comes from your authentication logic
    full_name: "",
    email: "",
    gender: "",
    date_of_birth: "",
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        ...
        navigate("/patient-dashboard"); // Redirect to patients list after success
      } catch (error) {
        console.error("Error creating patient profile:", error);
        if (error.response) {
          setMessage(
            `Error: ${
              error.response.data.message || "Failed to create patient profile. Please try again."
            }`
          );
        } else {
